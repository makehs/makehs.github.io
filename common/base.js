...
function updateTime() {
  var d = new Date(new Date().getTime() + 288e5)
    .toISOString()
    .replace("T", " ");
...
function setDynamicTime(selector, start = 0, end = 19) {
  time_update_list.push({ selector: selector, start: start, end: end }),
    updateTime(),
    time_updater_init ||
      ((time_updater_init = !0), window.setInterval(updateTime, time_interval));
}
...
function setStaticTime(
  ...
  for (const element of document.querySelectorAll(selector)) {
    var hours = element.attributes["data-traceback-hours"]?.value
        ...
        : traceback_hours + (Math.random() - 0.5) * traceback_range,
      hours = new Date(new Date().getTime() + 288e5 - 3600 * hours * 1e3)
        .toISOString()
        .replace("T", " ");
...
const presetFilters = {
    name: (x) =>
      2 == x.length
        ? x[0] + "*"
        : x[0] + "*".repeat(x.length - 2) + x.slice(-1),
    lastnameonly: (x) => x[0] + "*".repeat(x.length - 1),
    firstnameonly: (x) => "*" + x.slice(1),
    lastcharonly: (x) => "*".repeat(x.length - 1) + x.slice(-1),
    idcard:
      (start = 2, end = 2, mask = 18 - start - end) =>
      (x) =>
        x.slice(0, start) + "*".repeat(mask) + x.slice(-end),
    phone:
      (start = 3, end = 4, mask = 11 - start - end) =>
      (x) =>
        x.slice(0, start) + "*".repeat(mask) + x.slice(-end),
  },
  fields = {};
function addStorageField(
  ...
  for (const element of elements)
    (element.innerText = filter(init_value)),
